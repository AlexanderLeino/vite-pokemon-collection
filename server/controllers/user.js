...
module.exports = {
  ...
  updateCardList: async ({ body }, res) => {
    try {
      ...
      let foundUser = await User.findOne({ _id: userId }).elemMatch("cards", {
        name: cardData.name,
  ...
  findCardSubDoc: async ({ body }, res) => {
    ...
    try {
      User.findOne({ _id: body.data })
        .elemMatch("cards", { name: "Umbsdfreon", cardNumber: "215" })
        .select("cards.$")
        .exec(async function (err, doc) {
              if(doc) {
                ...
                res.status(200).send("Congrats your user has been updated!")
              }
                else {
                  throw new Error("Couldnt Find Specified Card on User :/")
                }
        });
